...
resource "aws_iam_role" "ingress_role" {
  name = "kf-admin-${var.cluster_name}-role"
...
resource "kubectl_manifest" "ingress-cognito" {
...
resource "kubectl_manifest" "ingress-params" {
  yaml_body = <<YAML
apiVersion: v1
data:
  loadBalancerScheme: internet-facing
...
data "kubectl_file_documents" "alb" {
  content =file("${path.module}/alb.yaml")
}
...
resource "kubectl_manifest" "alb-config" {
yaml_body = <<YAML
apiVersion: v1
data:
...
data "kubectl_file_documents" "profile" {
  content =file("${path.module}/profiles.yaml")
}

resource "kubectl_manifest" "profile" {
    for_each  = data.kubectl_file_documents.profile.manifests
    yaml_body = each.value
}
