...
variable "alarm_sns_topic_arns" {
  description = "A list of SNS topic ARNs to notify when the ECS Cluster alarms change to ALARM, OK, or INSUFFICIENT_DATA state"
  type        = list(string)
}
...
variable "ecs_cluster_high_memory_utilization_treat_missing_data" {
...
variable "ecs_cluster_high_cpu_utilization_treat_missing_data" {
  description = "Sets how this alarm should handle entering the INSUFFICIENT_DATA state. Must be one of: 'missing', 'ignore', 'breaching' or 'notBreaching'."
  type        = string
  default     = "missing"
}
