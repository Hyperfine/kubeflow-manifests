...
resource "aws_cloudwatch_metric_alarm" "tg_low_healthy_host_count" {
  ...
  dimensions = {
    ...
    TargetGroup  = data.template_file.target_group_dimension_value.rendered
  }
...
data "template_file" "target_group_dimension_value" {
  template = element(split(":", var.target_group_arn), 5)
}
