...
resource "aws_cloudwatch_metric_alarm" "alb_high_active_connection_count" {
  ...
  dimensions = {
    LoadBalancer = data.template_file.load_balancer_dimension_value.rendered
  }
...
data "template_file" "load_balancer_dimension_value" {
  template = replace(element(split(":", var.alb_arn), 5), "loadbalancer/", "")
}
