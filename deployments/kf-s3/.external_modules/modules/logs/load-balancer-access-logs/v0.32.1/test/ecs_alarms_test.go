...
func TestEcsAlarms(t *testing.T) {
	...
	test_structure.RunTestStage(t, "setup", func() {
		...
		terraformOptions.RetryableTerraformErrors[".*ClusterContainsContainerInstancesException.*"] = "Failed to destroy the cluster due to lingering services."

		// Additional retryable errors specific to ECS
		terraformOptions.RetryableTerraformErrors["Unable to assume role and validate the listeners configured on your load balancer"] = "An eventual consistency bug in Terraform related to IAM role propagation and ECS. More details here: https://github.com/hashicorp/terraform/issues/4375."
		terraformOptions.RetryableTerraformErrors["Error creating launch configuration: ValidationError: You are not authorized to perform this operation."] = "An mysterious, intermittent error that has been happening with launch configurations recently. More details here: https://github.com/hashicorp/terraform/issues/7198"
		terraformOptions.RetryableTerraformErrors["does not have attribute 'id' for variable 'aws_security_group."] = "An mysterious, intermittent error that has been happening with launch configurations recently. More details here: https://github.com/hashicorp/terraform/issues/2583"
		terraformOptions.RetryableTerraformErrors["Invalid IamInstanceProfile"] = "An intermittent error that happens with launch configurations. More details here: https://github.com/hashicorp/terraform/issues/2136"
