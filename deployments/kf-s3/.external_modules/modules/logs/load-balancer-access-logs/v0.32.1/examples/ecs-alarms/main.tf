...
EOF
  ...
  # Make sure all the ECS cluster are deployed before deploying any ECS service resources. This is also necessary to
  # avoid issues on 'destroy'.
...
module "ecs_cluster_alarms" {
  ...
  # source = "git::git@github.com:gruntwork-io/terraform-aws-monitoring.git//modules/alarms/ecs-cluster-alarms?ref=v1.0.8"
...
module "ecs_service_alarms" {
  # When using these modules in your own templates, you will need to use a Git URL with a ref attribute that pins you
  # to a specific version of the modules, such as the following example:
  # source = "git::git@github.com:gruntwork-io/terraform-aws-monitoring.git//modules/alarms/ecs-service-alarms?ref=v1.0.8"
