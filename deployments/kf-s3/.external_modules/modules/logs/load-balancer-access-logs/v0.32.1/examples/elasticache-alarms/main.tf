...
module "redis_alarms" {
  ...
  # source = "git::git@github.com:gruntwork-io/terraform-aws-monitoring.git//modules/alarms/elasticache-redis-alarms?ref=v1.0.8"
  ...
  monitor_database_memory_usage_percentage = var.redis_monitor_database_memory_usage_percentage
  monitor_curr_connections                 = var.redis_monitor_curr_connections
  monitor_replication_lag                  = var.redis_monitor_replication_lag
}
...
module "memcached_alarms" {
  # When using these modules in your own templates, you will need to use a Git URL with a ref attribute that pins you
  # to a specific version of the modules, such as the following example:
  # source = "git::git@github.com:gruntwork-io/terraform-aws-monitoring.git//modules/alarms/elasticache-memcached-alarms?ref=v1.0.8"
