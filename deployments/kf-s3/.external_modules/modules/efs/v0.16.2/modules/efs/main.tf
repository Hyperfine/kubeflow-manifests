...
locals {
  efs_endpoints_with_access_policy = [
    ...
    if length(options.read_write_access_arns) + length(options.read_only_access_arns) > 0
  ]
  create_efs_policy = var.enforce_in_transit_encryption || length(local.efs_endpoints_with_access_policy) > 0
}
