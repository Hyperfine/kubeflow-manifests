...
variable "allow_connections_from_cidr_blocks" {
  description = "A list of CIDR-formatted IP address ranges that can connect to this DB. In the standard Gruntwork VPC setup, these should be the CIDR blocks of the private app subnets, plus the private subnets in the mgmt VPC. This is ignored if create_subnet_group=false."
  type        = list(string)
}
...
variable "engine_mode" {
  description = "The version of aurora to run - provisioned or serverless. Note: serverless currently only supports MySQL and Postgres."
  type        = string
  default     = "provisioned"
}
