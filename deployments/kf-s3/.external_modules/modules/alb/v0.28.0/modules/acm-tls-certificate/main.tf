...
resource "aws_acm_certificate" "cert" {
  ...
  provisioner "local-exec" {
    ...
    # Due to one Terraform bug (https://github.com/hashicorp/terraform/issues/28065), self.tags may be null, and the
    # lookup function returns an error if you try to use it with null. Therefore, we have to explicitly check for null
    # here. However, due to another Terraform bug (https://github.com/hashicorp/terraform/issues/24128), the logical
    # binary operators (e.g., &&) are not short-circuiting, so we have to do this using nested ternary expressions
    # instead, as ternary expressions are evaluated lazily.
    command = (
      self.tags != null
      ? tobool(lookup(self.tags, "run_destroy_check", false)) ? "${path.module}/wait-until-tls-cert-not-in-use.sh ${self.arn}" : "true"
      : "true"
    )
  }
...
locals {
  # Loop through var.acm_tls_certificates, merging default variables. Note that this omits the hosted_zone_id field,
  # which will be dynamically looked up using the hosted_zone_lookup local variable.
  ...
  # We use a separate dynamic lookup for the hosted zone ID for the ACM TLS certificates map to avoid dynamic
  # content in a for_each.
  hosted_zone_lookup = {
    for domain, cert in var.acm_tls_certificates :
    domain => (
      lookup(cert, "hosted_zone_id", "") != ""
      ? cert.hosted_zone_id
      : (
        lookup(var.domain_hosted_zone_ids, domain, "") != ""
        ? var.domain_hosted_zone_ids[domain]
        : data.aws_route53_zone.current[domain].zone_id
      )
    )
  }
  ...
  # The map of domain names to their domain validation options for use in looping through during route53 record creation.
  # We create a record for every certificate input that has create_verification_record set to true. Note that we have
  # two separate maps here:
  #  - A static map that contains the list of verification domains to create.
  #  - A dynamic map that contains the actual data.
  # This allows the verification data to depend on dynamic content: we can use the first map in the for_each/count call
  # without dynamic content, and pack the dynamic content in the second map which is looked up on the resource as
  # terraform iterates the for_each.
  dns_verification_records_to_create = {
  ...
  dns_verification_record_data = {
    ...
    domain => {
      ...
      zone_id = local.hosted_zone_lookup[source_domain]
    }
  }
...
data "aws_route53_zone" "current" {
  ...
  #
  # IMPORTANT: if you are requesting a a certificate for domain X, but you're attaching it to a hosted zone that is NOT named X
  # you must specify the hosted_zone_id in the var.acm_tls_certificates input.
  # For example, if you are requesting a wildcard certificate for *.test.gruntwork.in, and your hosted_zone_domain_name
  # is gruntwork.in,then you MUST provide the hosted_zone_id for gruntwork.in in your input map
  #
  # Please see this module's README.md for more information
  for_each = local.data_zone_lookups_to_perform

  # Lookup by ID if ID is available, otherwise by name.
  zone_id = lookup(var.domain_hosted_zone_ids, each.key, null)
  name    = lookup(var.domain_hosted_zone_ids, each.key, null) == null ? trimprefix(each.key, "*") : null
