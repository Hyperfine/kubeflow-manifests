...
func TestAcmTlsCertificate(t *testing.T) {
	...
	for _, testCase := range testcases {
		...
		t.Run(testCase.testName, func(t *testing.T) {
			...
			sanDomainName1 := fmt.Sprintf("acm-test-san1-%s.%s", uniqueID, DefaultDomainNameForTest)
			sanDomainName2 := fmt.Sprintf("acm-test-san2-%s.%s", uniqueID, DefaultDomainNameForTest)

			acmTLSCertificatesMap := make(map[string]interface{})

			acmTLSCertificatesMap[domainName] = map[string]interface{}{
				"subject_alternative_names":  []string{sanDomainName1, sanDomainName2},
