...
resource "aws_alb_listener" "http" {
  ...
  load_balancer_arn = data.template_file.alb_arn.rendered
...
# The ALB's ARN depends on the value of var.alb_access_logs_s3_bucket_name
data "template_file" "alb_arn" {
  # This will return the ARN of the ALB that is actually created. It works as follows:
  # - Make a list of 1 value or 0 values for each of aws_alb.alb_with_logs and aws_alb.alb_without_logs by adding the
  #   glob (*) notation. Terraform will complain if we directly reference a resource property that doesn't exist, but it
  #   will permit us to turn a single resource into a list of 1 resource and "no resource" into an empty list.
