...
resource "aws_route53_record" "cert_validation" {
  count   = var.create_resources && var.create_verification_record ? 1 : 0
...
resource "aws_acm_certificate_validation" "cert" {
  count                   = var.create_resources && var.verify_certificate ? 1 : 0
  certificate_arn         = local.certificate_arn
  validation_record_fqdns = [local.flattened_domains[0]["resource_record_name"]]
