...
def handler(event, context):
    ...
    if not target_db_snapshots:
        logger.info('None of the shared snapshots in this account are of RDS DB %s in account %s. Nothing to do.', db_identifier, db_account_id)
        return
    ...
    if not db_snapshots_to_copy:
        logger.info('All of the shared snapshots of RDS DB %s in account %s have already been copied to this AWS account. Nothing to do.', db_identifier, db_account_id)
        return
...
def filter_to_snapshots_not_yet_copied(rds_client, snapshots, db_is_aurora_cluster):
    logger.info('Filtering the following list of snapshots to those that have not already been copied into this AWS account: %s', snapshots)

    snapshots_not_yet_copied = []

    for snapshot in snapshots:
        if not local_copy_of_snapshot_exists(rds_client, snapshot, db_is_aurora_cluster):
            snapshots_not_yet_copied.append(snapshot)
...
def local_copy_of_snapshot_exists(rds_client, snapshot, db_is_aurora_cluster):
    ...
    logger.info('Looking up if a local copy of snapshot %s exists', snapshot_id)
...
def filter_to_snapshots_of_target_db(all_shared_snapshots, target_db_identifier, target_db_account_id, target_db_is_aurora_cluster):
    logger.info('Filtering the following RDS snapshots to those that are of RDS DB %s in AWS account %s: %s', target_db_identifier, target_db_account_id, all_shared_snapshots)
    target_snapshots = []

    for snapshot in all_shared_snapshots:
        if is_snapshot_of_target_db(snapshot, target_db_identifier, target_db_account_id, target_db_is_aurora_cluster):
...
def copy_snapshots(rds_client, db_snapshots_to_copy, db_is_aurora_cluster, kms_key_id):
    logger.info('Making local copies of snapshots: %s', db_snapshots_to_copy)
...
def copy_snapshot(rds_client, snapshot, db_is_aurora_cluster, kms_key_id):
    ...
    logger.info('Making a local copy of snapshot %s to %s', snapshot_arn, snapshot_id)
